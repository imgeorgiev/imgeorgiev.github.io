...
Now it's time to do the Taylor series approximation according to how we [defined it](eq:taylor-series). That is straightforward for <span> $g(\delta \textbf{x}_n + \bar{\textbf{x}}_n, \delta \textbf{u}_n + \bar{\textbf{u}}_n)$ </span> since it is a simple function. However,  <span> $V(\delta \textbf{x}_{n+1} + \bar{\textbf{x}}_{n+1}, t_{n+1})$  </span> is less straightforward as it depends on our dynamics $f$:
