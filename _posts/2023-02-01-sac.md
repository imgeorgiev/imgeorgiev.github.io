...
where $\alpha$ is an update step and $\hat{V}(s_t)$ is our new estiamte for $V(s_t)$ (that we're not fully certain of; that's why we call it estimate). OK, but how can we get an estimate? We can use the definition of the value function from [Eq. 3](#value_func) but then we still need to wait for the outcome of the episode. In [Eq. 4](#recursive_value_func) we discovered an interesting recursive relationship that only considers one step in the future. Swapping in that we get a new update rule:
...
TD-Learning is considered by many the core of RL! Armed with [Eq. 7](#td0_udpate), we can conceive a simple value estimator algorithm for our toy problem.
...
One way is to take our TD(0) update from [Eq. 7](#td0_udpate) and reformulate it with state-action values instead:
...
Let's for a moment say we can feasibly compute this, then why should we compute the expected value of $Q(s_{t+1}, a_{t+1})$ in [Eq. 8](#td0_q_update), why don't we directly optimize Q there which is guaranteed to lead to the optimal Q-value! Wait, what is the optimal Q-value?
...
Thus we can now reformulate [Eq. 8](#td0_q_update) to always follow the (currently) optimal action, not just to blindly use the sampled action:
...
Plugging this directly into [Eq. 12](#j_intermid)
...
1. A $\psi$-parametarised **critic** that lears a $Q(s,a; \psi)$ state-action value estimator based on Q-learning. We use our standard [Eq. 9](#q_learning_udpate) but now we get our optimal action from $a_t \sim \pi(a_t \| s_t; \theta)$. Thus our loss and update equations are now:
...
2. A $\theta$-parametarised **actor** which learns according to our [PG theorem Eq. 11](#policy_grad). The key here is that instead of optimizing for maximal reward, we can optimize for the surrogate of the reward, the Q-function!
